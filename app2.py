...
def outline_generator(state):
    outline_prompt =  f"""
        Generate the outline of an original {state['desired_pages']}-page fiction novel in the style of Olivia Cunning.
        Imagine and then carefully label the following: a detailed plot, characters with names, settings 
        and writing style.
        Make sure the characters are multi-dimensional, flawed, and relatable. Be sure to include steamy and passionate romance.
        Write sizzling chemistry and intimate scenes that leave readers breathless and eagerly turning pages. Also include
        excitement, drama, and tension in order to keep the reader hooked from start to finish.
        The writing style should be in the style of the author Olivia Cunning.
        Be sure that the plot and characters do not copy Olivia Cunning's previous works but instead take inspiration from them. 
        5000 words remaining to write the outline. It is CRITICAL you as many words as possible. DO NOT create a title!
        """
...
def state_populator(state):
    ...
    for (key, value) in items_to_populate.items():
        state_populator_prompt = f"""
            ...
            Be sure to include plenty of detail. Do not include anything in your response other than the {value}.
            Do not give an explanation of how you came to your result. Here is the outline: {state['raw_outline']}`
        """
...
def plot_summary_by_chapter(state):
    ...
    chapter_summary_prompt = f"""
        ...
        few chapters for dramatic twists in the plot and conclusion. Be sure to take inspiration from Olivia Cunning.
    """
...
def chapter_summary_array(state, starting_chapter=0):
    ...
    for i in range(starting_chapter, state['num_chapters']):
        ...
        chapter_summary_prompt = f"""
        You are writing a summary of chapter {i+1} of a {state['num_chapters']} chapters erotic novel. 
        ...
        You have to write up to 1000 words.`
        """
...
def page_generator(state, starting_chapter=0, starting_page=0):
    ...
    for i in range(starting_chapter, state['num_chapters']):
        for j in range(starting_page, state['chapter_length']):
            ...
            page_gen_prompt = f"""
                You are Cooleen Hoover writing page {j+1} in chapter {i+1} of a {state['num_chapters']}-chapter erotic
